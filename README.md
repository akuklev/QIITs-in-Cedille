...
The use of universal quantifier `∀` is justified by the fact that the type system is expressive enough to encode propositions (like “`n` is even” or “`n` is greater than `m`”) as types (`IsEven[n̲ : Nat]`, `IsGreater[n̲ m̲ : Nat]`) inhabited by proofs of these propositions, and a proof that a predicate `P[x̲ : X]` holds for each `x : X` is precisely a term of the type `∀x̲ : X, P[x]`. It is also true that for two propositions `A` and `B`, an constructive proof that `A` implies `B` is a term of the type `A -> B` (such term is the thing that yields a proof of `B` whenever we have a proof of `A`).
...
> ```
> Together with availability of automatic opaque unboxing of types of the kind `𝚺\Carrier : U, SomeStructureOn[Carrier↑]` we this syntactic sugar is precisely equivalent Path-Dependent Types as seen in the Scala language.
