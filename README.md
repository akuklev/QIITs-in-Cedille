...
We'll work in a system where terms don't in general have a canonical type, but can be typechecked against a given type, possibly nonunique. We'll write bare (untyped) lambda terms like
```
\x ↦ expr(x)
```
...
```
∀\n : Nat, List[length ↦ n] -> List[length ↦ n]
```
